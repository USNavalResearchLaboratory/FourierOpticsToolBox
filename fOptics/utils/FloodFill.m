...
r = 0; q = 0;
while unsuitable
    colref=round(c_dim/2+r*cos(q)); rowref=round(c_dim/2+r*sin(q));
    unsuitable = (~IM_mask(rowref,colref)) || (branch_cuts(rowref,colref));
    q = q+pi/5;
    r = q/2/pi;
end
...
count_limit=0;
adjoin_stuck=0;
while (sum(sum(adjoin(2:r_dim-1,2:c_dim-1)))~=0) && (count_limit<100)            %Loop until there are no adjoining pixels or they all lie on the border
